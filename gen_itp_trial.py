...
#can't remember why I added 0 at the end ^
...
var = data.iloc[1].drop(data.columns[[0,1]]).dropna(axis = 0).to_numpy() #obtain variable names
print(meta, var)
#all necessary info from top rows acquired, can drop these now
#print(data)
data = data.drop([1,2]) #drop top two rows, reindexing happens next
data = data.reset_index(drop = True)
#print(data)

data = data.dropna(axis = 1) #can finally drop the NaNs safely
#print(data)
'''
...
#data = data.reset_index(drop = True) #done above
#print(data)
data['times'] = pd.to_datetime(data[1], unit = 'D', origin = str(data[0][0]))
data = data.drop(data.columns[[0,1]], axis = 1)
#print(data)
#b[-1] = 'times'
#data.columns = #how to combine var with 'times'?
print(data)
