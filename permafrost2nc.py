...
def extractdatajson(frostcfg,station,stmd,output):
    ...
    if "validFrom" in sourcedata["data"][0].keys() and datetime.strptime(args.endday,"%Y-%m-%d") < datetime.strptime(sourcedata["data"][0]["validFrom"], "%Y-%m-%dT%H:%M:%S.%fZ"):
        ...
        return
    #check for multiple boreholes at station, should be solved by error-message and break.
    n = 0 #number to iterate over
    #use a while True? hmmm
    while True:
        nstation = ("{}{}{}".format(station,":",n))
        par = {"sources" : nstation, "elements" : element, 
               "referencetime" : ("{}{}{}".format(args.startday,"/",args.endday)),}
        mylog.info("Retrieving data for station: %s", nstation)
        try:
        ...
        if r.status_code == 412:
            mylog.error("Information returned indicates that no data is available for this time period for station %s", nstation)
            break
        ...
        #need a check for if Station Holder is STATENS VEGVESEN
        #print("skrabada")
        if sourcedata["data"][0]["stationHolders"] == ["STATENS VEGVESEN"]:
            for ts in data:
                for dic in ts["observations"]:
                    dic["level"] = {"levelType": "depth_below_surface","unit": "cm","value": 3} #add in the missing level dict
        ...
        ds.attrs["altitude"] = sourcedata["data"][0]["masl"] #reasonable to include?
        ...
        ds.attrs["history"] = ("{} {}".format(metadata["createdAt"],
                                              "Data extracted from the MET Observation Database through Frost and stored as NetCDF-CF"))
        ...
        outputfile = ("{}{}{}{}{}{}{}{}".format(output["destdir"],"/",nstation,
